...
class TestInterFace(ParametrizedTestCase):
    ...
    def test_case(self):
        ...
        try:
            ...
            raise e
        # print(f"• 服务ip：{res.raw._connection.sock.getpeername()}")
        # print(f"• 本地ip：{res.raw._connection.sock.getsockname()}")
...
def run(case_env, config, apis_list, verbosity=1, name=None, title=None, description=None, tester=None):
    ...
    for api in apis_list:
        setattr(TestInterFace.test_case, "__doc__", u"%s" % api['title'])  # 给用例函数设置doc 注释  体现在报告里为小用例标题
